...
When the pool is empty, new work waits. That wait is better than an unlimited storm of new connections.
...
1. What does a migration version record, and why must you not edit a migration that already ran in a shared environment?
2. What is the difference between an "up" script and a "down" script?
3. What is the difference between a seed and a fixture, and in which order do you run migrations and seeds?
4. Why must you not copy production personal data into a development seed?
5. When does reference data belong in a migration instead of a seed, and what does expand-then-contract mean for a column change?

#### Easy practical tasks

1. Name the migration tool that your language or framework uses, or a SQL-file convention. Write a two-file sequence: create `customers`, then add `email`.
2. Write a seed of three `products` rows for your practice schema. Write a fixture of one customer and two orders for a "list orders" test.
3. List three statements that belong in a migration and two that belong in a seed. Mark four files in a sample project as migration, seed, fixture, or application code.
4. Write two rules from this section that protect production. Write one reason to refuse `DROP TABLE` in a production migration without a backup.

#### Medium practical tasks

1. Run migrations on an empty database. Confirm the version table. Add a new migration. Run again. Attempt to run the same migration twice. Write how the tool prevents a double apply.
2. Run migrations and a seed on a disposable database. Drop the database. Repeat. Confirm that the process is repeatable.
3. Write an expand-contract plan for rename of `name` to `display_name` in three deployments. Separate demo users from required lookup rows (example: order statuses). Put each in the correct place.

#### Advanced practical tasks

1. Write a one-page migration standard: naming, review, backup, and forbidden edits. Include one unsafe example. Practice a failing migration (bad SQL) in a disposable database. Record how you repair forward with a new migration.
2. Design a seed that is idempotent (run twice, same end state). Implement it with `MERGE` / upsert or a clear-and-insert rule. Write a one-page data policy: what developers may seed, what tests may load, and how you anonymize a production sample if the team needs one.
