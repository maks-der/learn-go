...
This topic shows how applications talk to MongoDB. You use an official driver. You configure a connection string. You manage `MongoClient` for the process life cycle. You set timeouts. You learn trade-offs of object mapping libraries. You never run an unbounded `find` in an API.

Complete CRUD and querying first. Complete this topic before you study transactions in application code.
...
## Connection string, `MongoClient` lifecycle, and timeouts
...
2. Why do you create one client per process?
3. What does the client pool hold?
4. What is server selection timeout?
5. Why does a context deadline belong in an API handler?

#### Easy practical tasks

1. Connect with a URI from an environment variable. Do not print the password. Add `appName`.
2. Create a client, ping, list collection names, disconnect. Write the four calls in your language.
3. Find the timeout option names in your driver. Write three names.
4. Set a short server-selection timeout and point the URI at a closed port. Record the error.

#### Medium practical tasks

1. Set `maxPoolSize` to a small number. Run a few concurrent finds. Write what you observe (wait or error).
2. Intentionally create a client per request in a loop of 100. Watch connection count if you can. Then fix it.
3. Compare a failed connect (bad host) with a slow query. Write which timeout applies to each.
...
## Object mappers (Mongoose and others)
