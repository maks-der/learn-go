...
This topic shows how MongoDB splits one data set across many machines. A **sharded cluster** has **shards**, a **mongos** router, and **config servers**. Complete replication, modeling, and indexes first.
...
## Shard key choice (the hard part)
...
2. Take a collection that you modeled in topic 5. Propose a key. List two queries that scatter.
...
Moving a chunk uses resources on the source shard, the destination shard, and the network. Balance during a peak write period can add latency. Atlas manages much of this. You still watch move times.
...
3. Where does the cluster store the chunk map?
4. Why can a find without a shard key hit all shards?
5. Why can a chunk move add latency?

#### Easy practical tasks

1. Open the chunks and balancer pages. Write both URLs.
2. Write the role of config servers in two sentences.
3. Draw two shards and four chunks. Show one chunk move.
...
3. Write when you would stop the balancer for one hour. Write the risk if you leave it stopped.

#### Advanced practical tasks

1. Read balancer windows (active window). Propose a window for a region that peaks at 18:00. Write the config idea.
2. Trace one targeted find and one scatter find through `mongos` to shards. Write the steps.

---

## Hot shards and jumbo chunks
...
Do not ignore jumbo warnings. They grow.

Prevention is easier than repair. Pick cardinality and even frequency before you shard.
...
2. What is a jumbo chunk?
3. Why can a chunk refuse to split?
4. Why can the balancer fail to cool a hot shard?
5. How can one large tenant create both problems?

#### Easy practical tasks

1. Write three causes of a hot shard. Write three causes of a jumbo chunk.
2. Open the jumbo-chunk page. Write the URL.
3. Draw a ranged `_id` key with all new writes on shard B. Label "hot".
4. Make a table: symptom, metric, likely cause.

#### Medium practical tasks

1. In a lab, insert many documents with the same shard-key value. Inspect `sh.status()` for uneven chunks or jumbo notes.
2. Design a countermeasure for one huge tenant without a full reshard (for example a tenant-specific collection). Write the trade-off.
3. Read how MongoDB marks a chunk jumbo. Write the condition in your own words.

#### Advanced practical tasks

1. Plan a production response: jumbo chunk on the primary write shard. Include detect, mitigate, and long-term key change.
2. Compare refine shard key vs reshard vs migrate one tenant off. Write cost and downtime risk for each.

---

## Refinable / resharding (modern versions)
