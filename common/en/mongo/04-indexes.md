...
An index is a data structure that helps the server find documents without a full collection scan. This topic shows single-field indexes, compound indexes, unique indexes, and multikey indexes. You also learn TTL, text, geospatial, and hashed indexes at a high level. You hide indexes and you read `explain`.

Complete querying first. Complete this topic before you study aggregation and performance.
...
## Single-field and compound indexes (prefix rule)
...
Do not create a duplicate index on the same key pattern.
...
3. What is the prefix rule?
4. Why does `{ a: 1, b: 1 }` not replace an index for queries on `b` only?
5. Why do extra indexes slow writes?

#### Easy practical tasks

1. Create an index on `{ userId: 1 }`. Run `getIndexes()`. Write the name.
2. Create `{ userId: 1, createdAt: -1 }`. Find `{ userId: "u1" }` and sort `{ createdAt: -1 }`. Run `explain`. Write if the plan uses the index.
...
2. Compare `{ userId: 1, createdAt: -1 }` and `{ createdAt: -1, userId: 1 }` for a user-history query. Write which prefix matches.
...
2. How can two documents without `email` conflict on a unique `email` index?
3. What is a multikey index?
4. Why can you not compound-index two array fields in the usual case?
5. Why do large arrays make a multikey index expensive?

#### Easy practical tasks

1. Create a unique index on `email`. Insert one user. Insert the same email again. Record the error.
2. Create a unique compound index on `{ tenantId: 1, sku: 1 }`. Show that the same sku can exist in two tenants.
3. Index `tags`. Insert a post with three tags. Find one tag. Run `explain`.
...
3. Measure index size with `collStats` before and after you add 1000 tags across many documents. Write the sizes.

#### Advanced practical tasks

1. Read about unique indexes and `null` in your server version. Write the exact rule that you tested.
2. Read the multikey restrictions page. Write two restrictions in your own words.

---

## TTL, text, geospatial, and hashed indexes (high-level)
...
These other index types solve special queries. Learn the idea. Read the full pages before production use.

**Text index.** A text index supports `$text` search on string fields. It tokenizes words. It is not a full search engine. Atlas Search (Lucene) is stronger for product search. One collection can have at most one text index in many versions. Confirm for your version.

**Geospatial index.** `2dsphere` indexes GeoJSON points and shapes. You query `$near` and `$geoWithin`. Use it for "find places near this point". Store coordinates in the GeoJSON form that the manual shows.
...
2. What type must the TTL indexed field have?
3. What query operator uses a text index?
4. What is a `2dsphere` index for?
...
2. Create a text index on `title`. Insert two titles. Run a `$text` query. Write the matches.
3. Read the GeoJSON point example in the manual. Write one valid point document.
4. Make a table: index type, typical query, one limit. Add TTL, text, geospatial, hashed.

#### Medium practical tasks

1. Use `expireAfterSeconds: 60` on `createdAt`. Insert now. Write when you expect the delete.
2. Compare `$text` and `$regex` on the same field. Write which uses the text index.
...
2. Read Atlas Search vs text index. Write three features that Search adds.
...
2. Write a drop checklist: hide, watch metrics, explain slow queries, then drop. Run it on an unused test index.
