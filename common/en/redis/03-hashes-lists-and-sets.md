...
## Lists: push, pop, `LRANGE`, `LTRIM`, lists as queues
...
Reliable queues need more structure: a processing list, a timeout, and a retry. Redis Streams (topic 5 and topic 10) give consumer groups and acknowledgements. Use Streams when you need that model. Use a list when a lost job is acceptable or when a single worker is enough.
...
4. Which command pair gives FIFO on a list?
5. What happens to a job if a worker `RPOP`s and then the worker dies?

#### Easy practical tasks

1. `RPUSH lab:q x y z`. `LRANGE lab:q 0 -1`. Save the reply.
2. `LPOP lab:q`. `LRANGE lab:q 0 -1`. Save both.
3. `RPUSH` three items then `LTRIM lab:q 0 1`. `LRANGE` the list.
4. Enqueue three jobs with `LPUSH`. Dequeue with `RPOP` three times. Write the order.

#### Medium practical tasks

1. Build `1 2 3 4 5` with only `LPUSH` or only `RPUSH`. Show `LRANGE` and explain the order.
2. Open two `redis-cli` windows. `BRPOP` in one. `LPUSH` in the other. Record both sides.
3. Use `LMOVE` or `RPOPLPUSH` from `jobs` to `jobs:busy`. Document the two lists after the move.

#### Advanced practical tasks

1. Implement a "latest 20 events" key with `LPUSH` and `LTRIM 0 19` after each push. Prove the length stays 20.
2. Design a list-based retry: busy list, `LRANGE` of stuck items, requeue. Write the failure cases. Compare with Streams in five sentences.

---

## Sets: add, remove, membership, intersect, union
...
Sets are good for tags, unique visitors (when the set is not huge), and relationship indexes (`user:42:roles`). They are a poor unique-visitor store at internet scale. Use HyperLogLog (topic 5) when you need an approximate count and not the exact ids.
...
If you need rank or a numeric score, use a sorted set (topic 4). If you need an append-only log with ids, use a stream (topic 5).
...
1. Implement like/unlike with a set and `HINCRBY` on a post hash so that `likes` and membership stay consistent. Note the two-command race. Mention Lua (topic 6) as the fix.
