...
Do not picture the index as a copy of the full table. It stores the key and a pointer (or a subset of columns). The optimizer chooses whether to use the B-tree. You create the index. You do not pick the path in the `SELECT` text in standard SQL.
...
`DISTINCT` and `GROUP BY` also care about the number of distinct values (cardinality). Remember: few matches → index; most rows match → scan.

### Questions

#### Theoretical questions

1. What is selectivity, and why is `order_id = 1001` more selective than `status_code = 'closed'` in a typical shop?
2. Why can the same column have different selectivity for two values?
3. What does a covering index contain, and why can it skip the table row?
4. Why does `SELECT *` rarely get a covering index?
5. Why is an index on a boolean often a poor default, and what is the write cost of a wide covering index?

#### Easy practical tasks

1. Estimate selectivity: 100 matching rows in 1 million. Write the fraction. Label five predicates as high or low selectivity in a shop.
2. Write a query that an index on `(customer_id, ordered_at)` can cover. Add one extra selected column and write why the index may no longer cover.
3. Explain in four sentences why the optimizer needs statistics, and the lookup from a secondary index to the row.
4. Write one composite predicate that is more selective than each part. Mark "index only" as the plan phrase to look for (or the product equivalent).

#### Medium practical tasks

1. Count rows for `status_code = 'open'` versus a single `order_id`. Compare fractions. Read a plan for both filters if the table is large enough.
2. Create a narrow index. Select only those columns. Read the plan. Select one more column. Read the plan again. Write the difference.
3. Find the distinct count of a column (`COUNT(DISTINCT ...)`). Relate it to selectivity. Read whether your DBMS supports `INCLUDE` or an equivalent.

#### Advanced practical tasks

1. Write a one-page note: selectivity, cardinality, covering, and when you create an index. Show a case where an index exists but the optimizer scans because the estimate is high.
2. Design one covering index for a hot customer-order list query. Justify each column. Write a trade-off: covering index versus an extra summary table for a report.
