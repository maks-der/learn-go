...
This topic explains arithmetic, comparison, and logical operators. You learn `if` and `else`. You learn value switch and type switch. You learn `for` as the only loop. You also learn `break`, `continue`, labeled breaks, and `defer`.

Go has no `while` keyword. `for` is the only loop. Use `defer` to schedule a call that runs when the function returns.

Use one term for each concept. An operator produces a value from operands. A statement changes control or assigns a value. A deferred call runs after the surrounding function ends.
...
Bitwise operators work on integers. The operators are `&`, `|`, `^`, `&^`, `<<`, and `>>`. `^` is xor when it has two operands. `^` is bitwise complement when it has one operand.
...
Go 1.21 and later provide built-in `min` and `max` for ordered values.
...
## `for` as the only loop (`for`, `for range`)
...
Go has `goto`. `goto` jumps to a labeled statement in the same function. Do not use `goto` in new code. Use `for`, `return`, or a labeled `break`.
...
Named result parameters can change in a deferred function. Topic 4 covers that pattern. In this topic, use `defer` for simple cleanup and for print order.
