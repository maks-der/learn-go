...
This topic shows source and sink connectors, tasks, converters, single message transforms (SMT), dead letter queues, and common connectors (JDBC, S3, Debezium). Complete this topic after topics 4, 5, and 7. Connect workers talk to a Kafka cluster. Use KRaft for that cluster. Do not add ZooKeeper for new work.
...
- Avro, Protobuf, or JSON Schema converters that talk to a Schema Registry (topic 7)
...
Typical SMT actions: rename or drop a field, change a topic name (route), insert a static field, mask a value, flatten a nested structure.
...
SMTs do not replace stream processing. An SMT must stay simple. It has no window and no join across records. For aggregations, use Kafka Streams, ksqlDB, or Flink (topic 9).
...
Do not put secrets in SMT static fields. Do not use an SMT as the only access-control mechanism. Topic 10 covers ACLs.
...
4. How is a converter different from an SMT?
5. Why is an SMT not a stream processor?
...
4. Draw a source path: external row → converter → optional SMT → Kafka bytes.
...
3. Apply one official SMT (for example, insert a field or change a topic) on a lab connector. Show a record before and after.

#### Advanced practical tasks

1. Use an Avro converter with a Schema Registry and a KRaft Kafka cluster. Write the subject name that you see.
2. Build a two-SMT chain that masks one field and routes to a second topic. Write the configuration and two sample records.
...
Topic 3 describes poison messages on the producer path. Topic 11 describes poison messages in applications. The Connect DLQ is the operational tool on the Connect path.
...
2. Compare Connect DLQ with application poison-message handling from topic 3 and topic 11. Write five similarities and three differences.

---

## JDBC, S3, and Debezium (CDC)
...
CDC is a good publisher for the outbox pattern (topic 11). The application writes an outbox row in the same database transaction. Debezium reads the log and produces to Kafka.
...
1. Design a path: Debezium or JDBC source → Kafka topic → S3 sink. Write converters, a possible SMT, and a DLQ topic. Do not invent product keys that you did not read.
...
1. Describe the path of one database row from a source connector to a sink file. Name connector, task, converter, optional SMT, topic, and sink.
...
4. What stays the same when Connect moves from a local KRaft broker to a managed Kafka service?
5. How does topic 7 (schema) change the converter choice for Connect?

#### Easy practical tasks

1. Write a one-page cheat sheet: source, sink, task, converter, SMT, DLQ, JDBC, S3, Debezium.
