...
## Cross join and when it is a mistake
...
3. How does a comma `FROM` list relate to a cross join?
4. Why is a forgotten `ON` clause dangerous?
5. Why must you not test a cross join on two large tables?

#### Easy practical tasks

1. Cross-join two tiny lookup tables (two rows each). Show all four pairs.
2. Write the row-count formula for a cross join.
3. Rewrite a comma `FROM` of two tables as `CROSS JOIN`.
4. Explain in four sentences when a cross join is a mistake.

#### Medium practical tasks

1. Build a size-and-color grid with `CROSS JOIN`. Insert the result into a scratch table if you want.
2. Forget `ON` on a join of `orders` and `customers` (or use comma syntax). Stop the query if it grows. Write the lesson.
3. Compare run time of an inner join and a cross join on the same two tables (keep tables small).

#### Advanced practical tasks

1. Write a one-page note: generate a calendar of dates with a cross join of year, month, and day lookups, then filter valid days.
2. Find a production incident story (public) about a missing join condition. Write five sentences on the cause.
...
- compare a row to an earlier row (same customer, previous order)
- find pairs in a list (airports, `from_code` and `to_code` in one table of cities)

You must alias the table twice. Without aliases, the DBMS cannot tell the two roles apart.

Do not create a loop in your head as a recursive walk unless you use a recursive query. A single self-join walks one step (employee to manager). A full chain needs more joins or a recursive common table expression. That feature is later material.
...
2. Why do you need two aliases?
3. What does `ON e.manager_id = m.employee_id` mean?
4. When do you use a left self-join for employees?
5. How many management steps does one self-join walk?

#### Easy practical tasks

1. Create a small `employees` table with a manager column. Insert three rows.
2. Write an inner self-join that lists employee and manager names.
3. Write a left self-join that includes the top manager.
4. Draw the three-row example from this section.

#### Medium practical tasks

1. Find pairs of customers in the same city with a self-join. Avoid pairing a row with itself (`a.customer_id < b.customer_id`).
2. List employees who share a manager.
3. Explain in six sentences a self-join on `flights` from city to city using two aliases.
...
## Join on keys vs join on expressions

A join on keys matches a foreign key to a unique key. The match is exact. The optimizer can use indexes (topic 11). The meaning is "these rows belong together."
...
A join on an expression matches a computed condition. Examples: date ranges, inequality, transformed text.
...
This join can match one order to many discount rows if the ranges overlap. The grain can explode. You must know the rule: one discount, or many, or the best match.
...
This join finds duplicate emails that differ only by case. The expression `LOWER(email)` can prevent a simple index use on some products.

Prefer a key join when the model has a key. Use an expression join when the rule is a range or a transformation. Document the expected match count: one-to-one, one-to-many, or many-to-many.

Do not join on rounded timestamps or on `LIKE` as a first design. Those joins are hard to test and hard to speed up.

If two conditions belong in the join, put both in `ON`. Do not hide a join rule in `WHERE` when you write an outer join (see the left-join trap).

### Questions

#### Theoretical questions

1. What does a key join match?
2. What is a join on an expression?
3. Why can a range join return more than one match?
4. Why can `LOWER(email)` change index use?
5. When do you prefer a key join?

#### Easy practical tasks

1. Write a key join between `order_lines` and `products`.
2. Write a range join in words for orders and discount windows.
3. Label three joins in a shop as key or expression.
4. Explain in four sentences the grain risk of a range join.

#### Medium practical tasks

1. Create `discounts` with date ranges. Join orders into ranges. Show an order that hits two ranges if you allow overlap.
2. Find duplicate names with a case-insensitive expression join on a small table.
3. Move a filter from `WHERE` to `ON` on a left join. Write the meaning change.

#### Advanced practical tasks

1. Design a rule that assigns exactly one price list to an order date. Write the join and the uniqueness constraint idea.
2. Write a one-page note: when an expression join is a sign that the model needs a key.
...
Prefer `EXISTS` over `IN` when the inner query can return `NULL` in a `NOT IN` trap (topic 4). Prefer a join when you need columns from both tables in the result. Prefer a subquery when you only filter.
...
2. Read a query plan for `EXISTS` versus `IN` on your DBMS (preview of topic 11). Write three observations.
...
3. What is the difference between a key join and a range join for result grain?
