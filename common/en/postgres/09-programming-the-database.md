# 9. Programming the Database

## Description

This topic shows how you put logic in PostgreSQL 16 and PostgreSQL 17. You learn SQL functions, PL/pgSQL, triggers, generated columns, and extensions. You also learn common extensions: `pgcrypto`, `citext`, `pg_trgm`, and `pg_stat_statements`.

Complete topic 8 first. You can grant `EXECUTE` and you know `SECURITY DEFINER` risk. Complete this topic before you partition large tables.
...
## SQL functions and PL/pgSQL

A SQL function body is one SQL statement or a list of statements. The language name is `sql`. The function runs in the same transaction as the caller.
...
Prefer SQL functions when the body is a query. The planner can often inline a simple SQL function.

PL/pgSQL is the procedural language that ships with PostgreSQL. You write blocks with `BEGIN` ... `END`. Dollar-quoting holds the body.
...
A block can declare variables. `INTO STRICT` requires exactly one row. `EXCEPTION` handles named conditions. `RAISE` reports an error.
...
Do not mark a function `IMMUTABLE` if it calls `now()` or reads a table. Do not put an empty `EXCEPTION WHEN OTHERS` that hides every error. Do not loop row by row when one SQL statement can do the work.

Official: [https://www.postgresql.org/docs/17/xfunc-sql.html](https://www.postgresql.org/docs/17/xfunc-sql.html), [https://www.postgresql.org/docs/17/plpgsql.html](https://www.postgresql.org/docs/17/plpgsql.html).

### Questions

#### Theoretical questions

1. What is the difference between `STABLE` and `IMMUTABLE`?
2. Why can a simple SQL function be good for the planner?
3. Why does a function body use `$$` or `$tag$`?
4. What does `INTO STRICT` require?
...
2. Create `shop.clamp_qty`. Select it for `-1`, `NULL`, and `5`.
3. Run `\df shop.*` and `\sf shop.item_label`.
4. Create a SQL function that returns the count of rows in one table.

#### Medium practical tasks

1. Create a `RETURNS TABLE` function. Select from it with `SELECT * FROM shop.low_stock(5);`.
2. Write `shop.next_label` with `INTO STRICT` and two handlers. Test missing id and one existing id.
...
2. Write a PL/pgSQL function that updates one row and returns `RETURNING` data through `INTO`. Handle `NO_DATA_FOUND`.
...
In PostgreSQL 16 and 17 the keyword is `EXECUTE FUNCTION` (older text says `EXECUTE PROCEDURE`). Prefer `EXECUTE FUNCTION`.
...
`updated_at` defaults do not change on `UPDATE`. A trigger or an explicit `SET` is the usual fix.

Do not use triggers to replace a foreign key or a check constraint. Do not stack many triggers without a documented order.

Official: [https://www.postgresql.org/docs/17/triggers.html](https://www.postgresql.org/docs/17/triggers.html).
...
A generated column is not a trigger. The planner sees a real column. `SELECT *` includes it. A stored generated column plus a B-tree can replace a long expression index when you also select the value.
...
Official: [https://www.postgresql.org/docs/17/ddl-generated-columns.html](https://www.postgresql.org/docs/17/ddl-generated-columns.html).
...
Official: [https://www.postgresql.org/docs/17/extend-extensions.html](https://www.postgresql.org/docs/17/extend-extensions.html).
...
3. Show the schema of an extension with `\dx+`. Write where the objects live.
...
2. Read how `shared_preload_libraries` relates to `pg_stat_statements`. Write the extra step that `CREATE EXTENSION` alone does not do.

---

## Common extensions: `pgcrypto`, `citext`, `pg_trgm`, `pg_stat_statements`
...
**`pg_trgm`** adds trigram similarity and can index `LIKE '%x%'` with GIN or GiST.
...
**`pg_stat_statements`** records a normalized query and its total time. Topic 5 introduced it. You must load the library in `postgresql.conf`:

```text
shared_preload_libraries = 'pg_stat_statements'
```

Restart the server. Then `CREATE EXTENSION pg_stat_statements` in each database that must query the view.

`uuid-ossp` still provides other UUID versions. Create it only when you need those functions. It is not in the required list for this path.

Do not store reversible encryption keys in the same table as the ciphertext without a key plan. Do not enable `pg_stat_statements` and then ignore `pg_stat_statements.track`.
...
2. What problem does `citext` solve?
3. What index class does `pg_trgm` often use for `LIKE '%x%'`?
4. Why is `CREATE EXTENSION pg_stat_statements` not enough by itself?
5. When do you use `pgcrypto` instead of the built-in UUID function?
...
2. Read the 17 contrib pages for these four extensions. Write a table: extension, one function or view, when your app needs it.
...
2. Write a cheat sheet: SQL function, PL/pgSQL, trigger timings, `STORED`, `CREATE EXTENSION`, the four common extensions.
