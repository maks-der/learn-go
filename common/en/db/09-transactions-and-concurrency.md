...
This topic shows how the DBMS keeps writes safe when more than one session works at the same time. You learn ACID, `BEGIN` / `COMMIT` / `ROLLBACK`, autocommit, isolation anomalies, isolation levels, locks, and deadlocks.
...
**Durability.** After `COMMIT` succeeds, the committed data survives a crash. The DBMS writes a log to stable storage. A later topic covers the log in more detail. For now: commit means the change is persistent.
...
## Locks (row, table) at a conceptual level
...
Deadlocks are the next section. Locks are the resource that deadlocks use.
...
1. What does a lock reserve?
2. What does a row lock cover?
3. When do table locks often appear?
4. What happens when two sessions update the same row?
5. What is `SELECT ... FOR UPDATE` for?

#### Easy practical tasks

1. In session A, begin and update one row. Do not commit. In session B, update the same row. Write what you observe.
2. Commit session A. Write what session B does next.
3. Draw a timeline of those two sessions.
...
2. Start a `ALTER TABLE` in one session on a practice table (small). See if another session blocks. Write the result.
3. Read the lock timeout setting of your DBMS. Write the name and the default.

#### Advanced practical tasks

1. Write a booking pattern: select seat for update, check free, update, commit. Stay conceptual if the syntax differs.
2. Read whether your DBMS uses versions (MVCC) so that readers do not wait for writers. Write six sentences.
...
2. How does the DBMS usually break a deadlock?
3. Why must you retry the whole transaction?
4. Why does a stable lock order help?
5. Why are deadlocks part of normal design under load?

#### Easy practical tasks

1. Draw the two-row deadlock timeline.
2. Write the retry rule in four sentences.
3. List four ways to reduce deadlocks.
4. Explain why the aborted session sees a rollback.

#### Medium practical tasks

1. Reproduce a deadlock with two sessions and two rows if you can. Record the error.
2. Rewrite a transfer so that both sessions lock ids in the same order.
3. Find the deadlock error code or message in your DBMS docs.

#### Advanced practical tasks

1. Write a retry policy: max attempts, backoff, which errors retry. Keep it product-aware but short.
2. Write a one-page incident note: how you would explain a deadlock abort to a beginner developer.
