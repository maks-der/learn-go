...
Complete topic 10 first. You can detach a partition and dump a table. Complete this topic before you tune client pools.
...
`pg_dump` runs a consistent snapshot of that database. Other databases in the cluster are not in the file. Open transactions that hold old snapshots can delay the dump (topics 6 and 7).
...
Official: [https://www.postgresql.org/docs/17/app-pgdump.html](https://www.postgresql.org/docs/17/app-pgdump.html).
...
## `pg_basebackup`, WAL archiving, and PITR
...
- `-c fast` — fast checkpoint before the copy

The role needs `REPLICATION` (or superuser). `pg_hba.conf` needs a `replication` line.

PostgreSQL 17 adds incremental physical backup. You take a full `pg_basebackup`, then later `pg_basebackup --incremental` against a prior manifest. `pg_combinebackup` builds a usable directory from the full backup plus incrementals. PostgreSQL 16 does not have this incremental tool set.
...
Recovery on PostgreSQL 16 and 17 uses config plus a signal file:
...
There is no `recovery.conf` in these versions.
...
Do not run two primaries from copies of the same backup without a recovery plan. Do not set `archive_command` to a command that can succeed without a real copy. Do not test PITR only in your head.

Official: [https://www.postgresql.org/docs/17/app-pgbasebackup.html](https://www.postgresql.org/docs/17/app-pgbasebackup.html), [https://www.postgresql.org/docs/17/continuous-archiving.html](https://www.postgresql.org/docs/17/continuous-archiving.html).
...
2. What must you keep besides a base backup for PITR?
3. Which signal file do you use for recovery that promotes?
4. What incremental feature exists in 17 that 16 does not ship?
5. Where did `recovery.conf` go in PostgreSQL 16 and 17?

#### Easy practical tasks

1. Open the `pg_basebackup` reference. List five options from this section.
...
4. Write four sentences: physical backup, WAL archive, PITR, incremental 17.

#### Medium practical tasks

1. Create a `REPLICATION` role. Add a `pg_hba` replication line in a lab. Run `pg_basebackup` to an empty folder.
2. In a lab, set `archive_mode` and a local `archive_command`. Switch WAL with `SELECT pg_switch_wal();`. List new files in the archive folder.
...
2. On a 17 lab only, read `--incremental` and `pg_combinebackup`. Write a team rule: which major version may use that pair.
...
Slots (`pg_create_physical_replication_slot`) keep WAL on the primary until the replica consumes it. A slot that nobody consumes fills the disk.
...
Conflicts: the primary can vacuum or update a row that a long stand-by query still needs. The stand-by can cancel the query (`max_standby_streaming_delay`). `hot_standby_feedback` tells the primary about stand-by snapshots. It reduces query cancels. It can delay vacuum on the primary (more bloat).

Do not write on the stand-by. Do not use streaming as a backup by itself. Do not mix a PostgreSQL 16 primary with a 17 stand-by. Do not set `hot_standby = off` on a replica that applications must query.
...
4. What does `hot_standby = on` allow?
...
4. Write four sentences: streaming, hot stand-by, lag, slot.
...
3. If you have a replica, run `SELECT 1;` and try `CREATE TABLE`. Record the error.
...
`wal_level` must be `logical` on the publisher. PostgreSQL 15 and later (so 16 and 17) can add column lists and row filters on publications.
...
Logical replication does not copy everything. Typical gaps: sequences (check your 16/17 version notes), large objects, DDL (you apply schema changes yourself), and some types.
...
1. On two lab databases, publish `shop.items` and subscribe. Insert on the publisher. Select on the subscriber.
...
**Cloud vendor HA** (Amazon RDS Multi-AZ, Google Cloud SQL HA, Azure, AlloyDB, Aurora PostgreSQL, Crunchy-managed) runs a vendor failover path. You learn the vendor runbook: who promotes, what the DNS name does, and what RPO/RTO the product states. Topic 14 lists those products.
