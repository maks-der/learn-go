# 6. Transactions and MVCC

## Description

This topic shows transactions and concurrency in PostgreSQL 16 and PostgreSQL 17. You learn why readers do not block writers, isolation levels, row locks, advisory locks, deadlocks, and snapshots.

Complete topic 5 first. You can read plans. Complete this topic before you study vacuum.

Use one term for each concept. A transaction is a group of statements that commit or roll back together. Isolation is how much one transaction sees of another. A snapshot is the set of transactions that are visible. A lock waits or fails when another session holds a conflicting lock. MVCC means Multi-Version Concurrency Control. The default isolation level is read committed.
...
A single statement outside `BEGIN` is still a transaction. It commits when it succeeds.
...
- Dead tuples stay until `VACUUM` (topic 7).
...
MVCC is not the same as "no locks". DDL takes stronger locks. `SELECT FOR UPDATE` takes row locks. Foreign keys take locks.

Do not use `SELECT` as a lock. If you must stop another writer, use `SELECT ... FOR UPDATE` or an `UPDATE`. Do not keep a `BEGIN` open for hours on a busy database.
...
Most transactional applications use read committed. It is the right default. You cannot read uncommitted data from another transaction. There is no dirty read.

**Repeatable read** takes a snapshot at the first query. All later `SELECT` statements see that snapshot. If you `UPDATE` a row that another transaction committed after your snapshot, PostgreSQL can raise `could not serialize access due to concurrent update`. You roll back and retry.

Repeatable read in PostgreSQL uses snapshot isolation. Some write-skew anomalies can still occur.

**Serializable** uses Serializable Snapshot Isolation (SSI). If a dangerous pattern appears, one transaction fails with a serialization error (`SQLSTATE 40001`). The application must retry. All sessions that need the guarantee must use `SERIALIZABLE`.
...
Do not raise the global default to `serializable` without a plan for retries. Do not assume `READ UNCOMMITTED` gives dirty reads. It does not. Do not ignore serialization errors.
...
2. Start a transaction at `REPEATABLE READ`. Show the setting. Commit.
3. Start `SERIALIZABLE`. Show the setting. Commit.
...
3. Read the official isolation table. Write dirty, nonrepeatable, and phantom in your own words.

#### Advanced practical tasks

1. Write a two-session script that produces `could not serialize access due to concurrent update` under `REPEATABLE READ`.
2. Implement a retry loop in a language that you know: on `40001`, retry up to three times. Use a practice `UPDATE`.
...
The lock lasts until the end of the transaction. `COMMIT` or `ROLLBACK` releases it. `FOR UPDATE` does not lock the whole table.
...
`pg_try_advisory_lock` returns `false` instead of waiting. Two-argument forms use a pair of `int4` keys.

Uses for advisory locks: a single-job cron, a resource that has no row yet, work that spans more than one table.
...
Do not use advisory locks instead of a unique constraint. Do not use them instead of `FOR UPDATE` when a row exists. Session advisory locks must have a matching unlock on every path.
...
2. When is the row lock released?
3. What does `SKIP LOCKED` do?
4. What is the difference between `pg_advisory_lock` and `pg_advisory_xact_lock`?
5. When is an advisory lock a better fit than `FOR UPDATE`?
...
3. Take `pg_advisory_lock(1)`. Query `pg_locks` for `advisory`. Unlock.
4. Use `pg_try_advisory_lock` from two sessions on the same key.

#### Medium practical tasks

1. Build a two-row job table. Use `FOR UPDATE SKIP LOCKED LIMIT 1` from two sessions. Confirm they claim different rows.
2. Repeat `FOR UPDATE` with `NOWAIT`. Record the error.
3. Simulate a cron lock: session A holds 1001; session B tries and skips.
...
The other transaction continues. The aborted session must retry. The `SQLSTATE` is `40P01`.
...
- Vacuum and wraparound use xids (topic 7)
...
A long-running transaction holds an old snapshot. Vacuum cannot remove tuples that this snapshot still needs. That fact links this topic to bloat (topic 7).

Do not use xids as application keys. Xids wrap. Do not compare xids as integers in app code.
...
1. In `REPEATABLE READ`, select `pg_current_snapshot()` twice around a write in another session. Write if the snapshot text changed.
2. Compare `txid_current_snapshot()` with `pg_current_snapshot()` if both exist. Write which name you will use.
3. Open a long transaction that only `SELECT`s. In another session, update a row many times. Watch `n_dead_tup` (topic 7 preview).

#### Advanced practical tasks

1. Read "Transaction Isolation" and snapshot notes in the 17 docs. Write six sentences on `xmin`, `xmax`, and in-progress xids.
2. Explain in one page why a forgotten `idle in transaction` session blocks vacuum. Use snapshot words.
...
1. How do MVCC, a snapshot, and a row lock answer three different questions?
2. Why is read committed enough for most application writes?
3. When do you pick `FOR UPDATE SKIP LOCKED` instead of an advisory lock?
4. How do deadlock retry and serialization-failure retry differ?
5. Why does a long snapshot harm later vacuum even when no deadlock occurs?

#### Easy practical tasks

1. Run one script: `BEGIN`, `SELECT` with `xmin`/`ctid`, `UPDATE`, `SELECT` again, `COMMIT`. Save both `ctid` values.
2. Write a cheat sheet: MVCC, four isolation levels, `FOR UPDATE`, advisory lock, deadlock, snapshot.
3. `SHOW` `transaction_isolation` and `deadlock_timeout`.
4. Draw two writers on one row and a reader on the same row. Mark who waits.

#### Medium practical tasks

1. With two sessions, demonstrate: reader does not wait, writer waits, then a deadlock on two ids. Save client messages.
2. Claim a job with `FOR UPDATE SKIP LOCKED` and protect a cron with `pg_advisory_xact_lock` in the same lab notes.
3. Compare the same report query in read committed and repeatable read while a second session writes. Write the two result sets.

#### Advanced practical tasks

1. Write application rules for isolation, row locks, lock order, and retry `SQLSTATE` values (`40001` and `40P01`).
2. Read "Concurrency Control" in the 17 docs end to end. Map each heading to a section of this topic. Add one fact that this topic did not include.
