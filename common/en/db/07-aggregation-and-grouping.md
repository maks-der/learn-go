...
This topic shows how you compute totals and how you group rows. You learn aggregate functions, `GROUP BY`, and `HAVING`. You also learn a high-level idea of grouping sets and how `NULL` behaves in aggregates.
...
## Grouping sets idea (high-level)

A grouping set is one list of grouping columns. A simple `GROUP BY a, b` is one grouping set: `(a, b)`.

Sometimes a report needs more than one grouping set in one result:

- totals per country
- totals per country and city
- one grand total

You can run three queries and `UNION ALL` them. SQL also offers grouping-set syntax:
...
`()` is the empty grouping set: one grand total. In those rows, `country` and `city` are `NULL` (or a grouping marker). The `NULL` here means "all values," not "unknown city." That meaning is a common source of confusion.

Related syntax (high-level):

- `ROLLUP (country, city)` builds `(country, city)`, `(country)`, and `()`.
- `CUBE (country, city)` builds all combinations of those columns, including `(city)` alone.

Not every DBMS supports these clauses. If your product does not, write separate queries and `UNION ALL`.
...
Do not treat grouping-set `NULL` as missing data. If `city` can also be a real `NULL`, you need a grouping indicator (`GROUPING(city)` in products that have it).

Learn the idea. Use simple `GROUP BY` first. Add `ROLLUP` when a report needs subtotals and your product supports it.
...
2. What does the empty grouping set `()` compute?
3. What does `ROLLUP` add to a simple group?
4. Why can `NULL` in a `ROLLUP` row mean "all values"?
5. What do you do when the DBMS has no `GROUPING SETS`?

#### Easy practical tasks

1. Write three separate `GROUP BY` queries for city, country, and grand total in words or SQL.
2. Sketch a result with subtotal rows and a grand total row.
3. Explain `ROLLUP (country, city)` as a list of grouping sets.
4. Write four sentences on the two meanings of `NULL` in such a report.

#### Medium practical tasks

1. Build a subtotal report with `UNION ALL` of two grouped queries on practice data.
2. Try `ROLLUP` or `GROUPING SETS` on your DBMS. Record whether it works.
3. Mark subtotal rows in the `UNION ALL` form with a label column (`grain`).

#### Advanced practical tasks

1. Compare `ROLLUP` and `CUBE` in the official docs. Write six sentences and one example each.
2. Write a one-page warning: real `NULL` cities versus rollup `NULL`. Mention `GROUPING` if the product has it.
...
If you store `0` instead of `NULL` for "no measurement," `AVG` and `SUM` change. That choice is a model decision (topic 2). Do not mix `0` and `NULL` for the same meaning.
...
3. When is a grouping set or `UNION ALL` of totals the right report shape?
4. How do `NULL` values change `SUM`, `AVG`, and `GROUP BY`?
5. A teammate puts `COUNT(*) > 1` in `WHERE`. Which clause do they need, and why?

#### Easy practical tasks

1. Write a cheat sheet: five aggregates, `GROUP BY`, `HAVING` versus `WHERE`, `NULL` rules, grouping-set idea.
...
3. Add a grand total row with `UNION ALL` or `ROLLUP`.
