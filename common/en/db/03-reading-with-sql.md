...
SQL is the language that you use to read relational data. This topic shows `SELECT`, filters, sort, limit, distinct rows, and aliases. You learn projection and selection as two different operations.
...
SQL is a declarative language. You state the result that you want. You do not state the scan algorithm. The DBMS chooses an access path.
...
Use `DISTINCT` when the question is about unique combinations. Do not use `DISTINCT` to hide a bad join that multiplied rows. Fix the join first.
...
2. Why must `LIMIT` come with `ORDER BY` for a "top n" report?
3. What is the standard form that many texts use instead of `LIMIT`?
4. What does `DISTINCT` remove, and which columns does it compare?
5. Why must you not use `DISTINCT` to hide a bad join?

#### Easy practical tasks

1. Select all products ordered by `name`. Then select the same rows in reverse name order.
2. Return the first three rows by `price` descending. Use `LIMIT` or `FETCH`.
3. Insert two customers in the same city. Select `city` with and without `DISTINCT`.
...
3. Create rows that differ only in a column that you do not select. Show that `DISTINCT` on the remaining columns collapses them.

#### Advanced practical tasks

1. Write a stable pagination idea with `ORDER BY` key columns. Explain why the sort keys must be unique.
2. Document `TOP`, `LIMIT`, and `FETCH` for two products. Write one portable recommendation for this path.

---

## Comparison, `AND` / `OR` / `NOT`, `IN`, `BETWEEN`, `LIKE`, `IS NULL`
...
A comparison with `NULL` is unknown, not true. `WHERE price = NULL` returns no row. Use `IS NULL`.
...
This form matches `OR` of equalities. A subquery can appear in `IN` (later topic).

**`BETWEEN`.** The value sits in a closed range. `x BETWEEN a AND b` means `x >= a AND x <= b`. Both ends are included. For timestamps, a pair of `>=` and `<` is often safer.
...
2. Why do you write parentheses when you mix `AND` and `OR`?
3. What does `IN` test, and is `BETWEEN` inclusive on both ends?
4. What do `%` and `_` mean in `LIKE`?
5. Why must you use `IS NULL` instead of `= NULL`?

#### Easy practical tasks

1. Select products cheaper than `10`. Then select a price range with `>=` and `<`.
2. Select rows that match one of two names with `OR`. Rewrite the same filter with `IN`.
...
2. Compare `BETWEEN` on a `DATE` column and on a `TIMESTAMP` column at month end. Write the surprise if you find one.
...
2. Demonstrate `NOT IN` with a `NULL` in the list if you can. Record the result. Write the safe alternative in words.
...
`c` is the alias for `customers`. After you set an alias, use that alias for columns in many products. Be consistent.

Use aliases when names are long or when you join the same table twice (self-join in a later topic). Use short names that still mean something (`c` for customers, `o` for orders). Do not use `t1`, `t2` when you can use a letter that matches the table.
