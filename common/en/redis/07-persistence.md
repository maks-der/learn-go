...
A snapshot is a full copy of the dataset at a moment. Large datasets take time and extra RAM during `BGSAVE` (copy-on-write). Topic 11 covers the fork cost. Here, know that `BGSAVE` is not free.
...
On start, Redis replays the AOF (and may load an RDB preamble in hybrid mode). The dataset matches the log, except for a tail that was not `fsync`ed.
...
3. What is the loss window of `appendfsync always` in the common description?
4. What is the loss window of `everysec`?
5. Does `OK` mean the write is on disk in `everysec` mode?

#### Easy practical tasks

1. Run `CONFIG GET appendonly`. Save the value.
2. Run `CONFIG GET appendfsync`. Save the value.
3. Make a three-row table: mode, speed, typical loss window.
4. Write four sentences: AOF vs RDB.

#### Medium practical tasks

1. Enable AOF on a lab instance. `SET` a key. Restart. `GET` the key. Record the steps.
2. On a lab instance, set `everysec`, write keys, kill `-9` the process (Docker kill). Restart. Count how many of the last writes remain. Repeat a few times.
3. Run `BGREWRITEAOF`. Watch `INFO persistence` rewrite fields change.

#### Advanced practical tasks

1. Compare latency of 1,000 `SET`s with `always` vs `everysec` vs `no` on local disk. Write three times.
2. Read Redis 7 multi-part AOF notes. Write how a directory of files differs from one `appendonly.aof`.
