# 8. Commands, Pipelines, and Transactions
...
2. Pipeline commands to two different keys on Redis Cluster (topic 11 preview). Record whether the client had to split by slot.
...
`EXEC` can return an error or a null reply when a `WATCH`ed key changed (next section).
...
3. What does `DISCARD` do?
4. Do later commands run if one command fails at execution time?
...
3. Type a bad command after `MULTI` (wrong arity). Try `EXEC`. Record what happens.
4. Write four sentences that describe the queue.
...
2. Queue `INCR` on a hash key (create a hash first). `EXEC`. Show which replies are errors and whether other queued `SET`s applied.
3. Compare a pipeline of two `INCR`s with a `MULTI`/`EXEC` of two `INCR`s. Write interleaving differences.

#### Advanced practical tasks

1. Read the official transactions page. Write the exact rules for errors before `EXEC` vs errors inside `EXEC`.
2. Implement a two-key transfer with `MULTI` `DECR` `INCR` `EXEC` without a check. Write why the balance can go negative. Fix with Lua in a later section.
...
`WATCH` plus `MULTI` is enough for "read a balance, write if still the same" on one instance. On Cluster, all keys in the transaction must live in the same slot (topic 11).

### Questions

#### Theoretical questions

1. What does `WATCH` cause `EXEC` to do when a key changes?
2. Does `WATCH` stop other clients from writing?
3. What commands clear a watch?
4. Why must `WATCH`, `GET`, and `MULTI` use the same connection?
5. Is `WATCH` a lock?

#### Easy practical tasks

1. `WATCH lab:w`, `GET lab:w`, `MULTI`, `SET lab:w 1`, `EXEC` with no other writer. Save `EXEC`.
2. Write the retry loop in five numbered steps (no code required).
3. Run `UNWATCH` after a `WATCH`. Write a one-sentence purpose.
4. Open the `WATCH` command page. Write the return type of a failed `EXEC`.

#### Medium practical tasks

1. Two terminals: client A `WATCH`s and waits. Client B `SET`s the key. Client A `EXEC`s. Record the null `EXEC`.
2. `WATCH` two keys. Change only one from another client. Confirm `EXEC` fails.
3. Show a pool bug in words: `WATCH` on connection 1, `EXEC` on connection 2. Write the result.

#### Advanced practical tasks

1. Write a client program that retries a `WATCH` increment up to 5 times under parallel workers. Count successes and retries.
2. Compare `WATCH` vs Lua for "increment only if value < 10". Write which tool you pick and why.

---

## Transactions are not rollback-SQL
...
## Lua scripts: `EVAL`, `EVALSHA`, atomic multi-key updates
...
- Scripts must be deterministic for replication. Avoid `TIME` in old modes unless you follow the documented pattern (`redis.replicate_commands` / effects in current versions). Read the scripting docs for your version.
