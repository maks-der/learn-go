...
## Streams (`XADD`, `XREAD`, consumer groups) — intro
...
`>` means "new messages for the group". Pending entries that a consumer did not acknowledge stay in the PEL (pending entries list). Another consumer can `XCLAIM` them. Later topics cover the full model.
...
## JSON / Search / TimeSeries (Redis Stack / modules) — survey
...
You can complete topics 1 through 11 with core Redis. Use Docker image `redis/redis-stack` when you practice this survey.
