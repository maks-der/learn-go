# 11. Vacuum, Bloat, and Maintenance

## Description

This topic shows why PostgreSQL 16 and PostgreSQL 17 need vacuum. You learn dead tuples, autovacuum, manual `VACUUM`, `VACUUM FULL`, `ANALYZE`, transaction id wraparound, and bloat.

Complete topic 10 first. You must know MVCC and snapshots. Complete this topic before you study roles and security.
...
## Why UPDATE/DELETE leave dead tuples
...
Heap-only tuple (HOT) updates can avoid a new index entry when the indexed columns do not change and the new tuple fits on the same page. Topic 19 covers HOT. Know that some updates cost less than others.

`INSERT` does not create a dead tuple. A rolled-back `INSERT` leaves a tuple that vacuum must clean.
...
## Manual `VACUUM` and `VACUUM FULL` (the latter rewrites the table)
...
`VACUUM (ANALYZE)` also updates statistics. That pair is common after a bulk load.
...
`VACUUM FULL` is also named `CLUSTER` without an index when you only want a rewrite. Prefer the explicit `VACUUM FULL` name when the goal is shrink.
...
4. `SHOW` is not required. List three `VACUUM` options from this section.
...
2. Compare `VACUUM FULL` with `REINDEX` (topic 8). Write which problem each command fixes.

---

## `ANALYZE`

`ANALYZE` samples the table and stores statistics for the planner (topic 9). Autovacuum runs analyze on a separate threshold from vacuum.
...
You can analyze specific columns. That is useful when one column needs a higher statistics target.

After a large `INSERT`, `COPY`, or `DELETE`, run `ANALYZE` (or `VACUUM ANALYZE`) before you trust `EXPLAIN`. `reltuples` in `pg_class` then matches the new size more closely.

`ANALYZE` does not remove dead tuples. `VACUUM ANALYZE` does both.

A freeze-related analyze is not the same as planner statistics. Do not skip planner `ANALYZE` because you ran vacuum.
...
`n_mod_since_analyze` shows how much changed since the last analyze.

Do not run `ANALYZE` in a tight loop. It samples I/O. After bulk load, once is enough until the data changes again.

### Questions

#### Theoretical questions

1. What does `ANALYZE` store?
2. Does `ANALYZE` remove dead tuples?
3. When must you run `ANALYZE` after a load?
4. What does `n_mod_since_analyze` mean?
5. How is `VACUUM ANALYZE` different from `ANALYZE`?

#### Easy practical tasks

1. Run `ANALYZE` on one table. Select `last_analyze` from `pg_stat_user_tables`.
2. Insert many rows. Select `n_mod_since_analyze`. Run `ANALYZE`. Select again.
3. Analyze two named columns only.
4. Write four sentences: sample, planner, autovacuum analyze, vacuum.

#### Medium practical tasks

1. Compare `EXPLAIN` row estimates before and after `ANALYZE` on a table that you just loaded.
2. Set `STATISTICS` on one column (topic 9). `ANALYZE`. Check `pg_stats`.
3. Run `VACUUM ANALYZE` and read `VERBOSE` output for both vacuum and analyze lines.

#### Advanced practical tasks

1. Read how `ANALYZE` samples (`default_statistics_target`). Write how a higher target changes plan quality and analyze time.
2. Write a post-load checklist: `COPY`, `VACUUM ANALYZE`, `EXPLAIN` a typical query, check `reltuples`.

---

## Transaction ID wraparound (must know the risk)
...
`xid8` (64-bit) functions in topic 10 do not remove 32-bit wraparound from the heap. Freeze still matters in PostgreSQL 16 and 17.
...
2. Query `age(relfrozenxid)` for your public tables.
...
- `REINDEX INDEX CONCURRENTLY` (topic 8)
- drop unused indexes
...
2. Write a cheat sheet: dead tuples, autovacuum, `VACUUM`, `VACUUM FULL`, `ANALYZE`, wraparound, bloat.
