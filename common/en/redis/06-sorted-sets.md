...
4. How do you include scores in a `ZRANGE` reply?
5. What is the modern replacement for `ZREVRANGE`?
...
3. `ZINCRBY` one member by `5`. `ZSCORE` again.
4. `ZREVRANGE 0 0` (or `ZRANGE REV`). Write the top member.

#### Medium practical tasks

1. Add two members with the same score. Show `ZRANGE` order and `ZRANK` of each.
2. Use `ZADD NX` and `ZADD XX` on the same member. Record both replies.
3. `ZRANGE` with `BYSCORE` if you already read the next section, or use ranks to get the top 2 only.
...
2. Test `ZADD GT` / `LT` if present. Write a rule: "update score only when the new score is better."
...
1. What does `ZRANGEBYSCORE` select on?
2. How do you make a bound exclusive?
3. What do `-inf` and `+inf` mean?
4. What does `ZREMRANGEBYSCORE` return?
5. Why must you add `LIMIT` on large ranges?

#### Easy practical tasks

1. `ZADD tw 10 a 20 b 30 c`. `ZRANGEBYSCORE tw 10 20`. Save the reply.
2. `ZRANGEBYSCORE tw (10 30`. Save the reply.
3. `ZCOUNT tw 15 35`. Save the reply.
4. `ZREMRANGEBYSCORE tw -inf 15`. `ZRANGE tw 0 -1`. Save the reply.

#### Medium practical tasks

1. Use `LIMIT` to read the first two members in a score range. Then read the next two (offset 2).
2. Compare `ZRANGEBYSCORE` with `ZRANGE ... BYSCORE` on your server. Write if both work.
3. `ZPOPMAX` twice. Write which members left and the remaining `ZRANGE`.

#### Advanced practical tasks

1. Insert 10,000 timestamp scores. Remove everything older than a cutoff with `ZREMRANGEBYSCORE`. Time the delete.
2. Read `ZRANGE` command history (BYSCORE, REV, LIMIT). Write a migration note from old commands to `ZRANGE`.
...
## Time-series-like sliding windows (simple cases)
...
For heavy time series, use RedisTimeSeries (topic 7 survey) or a purpose-built store. For simple rate limits and "recent ids", a sorted set is enough.
