...
This topic shows declarative partitioning in PostgreSQL 16 and PostgreSQL 17. You learn range, list, and hash partitions, partition pruning, indexes on partitioned tables, and how you archive old partitions.

Complete topic 9 first. You can create tables and functions. Complete this topic before you study backup and replicas.

Use one term for each concept. A partitioned table is the parent. A partition is a child table that holds a slice of the rows. The partition key is the column list that chooses the slice. Partition pruning skips partitions that cannot match the query. Declarative partitioning is `PARTITION BY` on `CREATE TABLE`. Inheritance is a different, older feature. Prefer declarative partitioning for new large tables. This handbook targets PostgreSQL 16 and PostgreSQL 17.

---

## Declarative range, list, and hash partitions
...
Unique keys and primary keys on a partitioned table must include the partition key. A global unique `id` alone is not valid on the parent in PostgreSQL 16 and 17.

Do not partition a small table. Partitioning adds planning cost. Do not insert a row that matches no partition and has no default; the statement fails. Do not use table inheritance to split a new fact table (topic 2).

Official: [https://www.postgresql.org/docs/17/ddl-partitioning.html](https://www.postgresql.org/docs/17/ddl-partitioning.html).
...
`REINDEX` can target the parent or one partition. Topic 5 covered `REINDEX` and `CONCURRENTLY`.

A foreign key from a normal table to a partitioned table is supported when the referenced unique key exists. Cross-partition unique keys that omit the partition key are not available.
...
Do not `DELETE` millions of old rows from a non-partitioned table if you can partition by date and drop a slice. Do not drop a partition before a backup that you tested. Topic 11 covers `pg_dump` of one table.
...
2. `pg_dump` a detached table (`-t shop.orders_2025_01`). Restore it under a new name.
...
2. Write a cheat sheet: RANGE, LIST, HASH, prune, parent index, detach.
