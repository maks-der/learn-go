...
This topic shows how MongoDB copies data across members. A **replica set** is a group of `mongod` processes that hold the same data set. One member is the **primary**. The other data members are **secondaries**. Complete CRUD, drivers, and transactions first.
...
## Replica set: primary and secondaries
...
3. How does a secondary get new data?
4. Why is an odd number of voting members common?
5. Why must a driver discover the primary instead of one fixed host?
...
3. Open the replica-set introduction in the manual. Write the URL.
4. Make a table: member type, holds data, accepts writes, can vote. Add primary, secondary, and arbiter.
...
2. Compare a standalone URI and a replica-set URI. Write the extra options (`replicaSet` or SRV).
3. Stop the primary in a test set. Write which member becomes primary and how long the gap is.

#### Advanced practical tasks

1. Read about voting and priority in `rs.conf()`. Set one member to priority `0`. Confirm it does not become primary.
2. Read why Atlas discourages arbiters. Write three sentences in your own words.
...
Do not run two independent replica sets that think they own the same data directory. That is not a supported failover method.

Protocol version and election settings belong in the replica-set configuration. Read the manual for your major version before you change them.

Atlas runs elections for you. You still must know that a failover can occur and that a write with `w: 1` can roll back.

### Questions

#### Theoretical questions

1. What does an election select?
2. Why can one surviving member in a three-member set not accept writes?
3. What does it mean when the primary steps down?
4. What does priority `0` prevent?
5. Why can an election cause a short write outage?

#### Easy practical tasks

1. In `rs.status()`, find `electionDate` or election-related fields. Write what you see.
2. Open the replica-set elections page. Write the URL and the majority rule in one sentence.
3. Draw three members. Mark votes. Show why two down members stop writes.
4. Write four sentences: election, majority, step down, priority.

#### Medium practical tasks

1. On a test replica set, run `rs.stepDown()`. Time the gap until `hello` shows a new primary. Write the seconds.
2. Stop two of three members. Attempt an insert. Write the error. Start the members again.
3. Compare failover time with Atlas metrics or `rs.status()` after a test step-down. Write the observation.

#### Advanced practical tasks

1. Read about `term` and election protocol in the current manual. Write a six-sentence summary.
...
## Read from secondaries and stale reads
...
2. Combine this section with write concern and causal consistency from topic 11. Write a one-page "read your writes" policy.
...
**Atlas** creates a replica set (or a sharded cluster of replica sets) when you create a cluster. You do not start `mongod` by hand. You do not edit `rs.conf()` for routine failover.

A typical Atlas replica set has three data-bearing members. Atlas places them for high availability. Atlas runs elections, storage, and monitoring.

You connect with an SRV URI:

```text
mongodb+srv://cluster0.example.mongodb.net
```

The driver resolves hosts. Hosts can change. Do not pin one Atlas IP as the only host.

Atlas features that relate to this topic:

- Automatic failover
- Configurable oplog size on many tiers
- Backups that are separate from the live replica set
- Optional extra nodes for analytics on some tiers

You still choose write concern, read preference, and whether the application accepts stale reads. Atlas defaults are often majority write concern. Read the cluster settings.

The Atlas free tier is enough to learn replica-set behavior from the client side. You cannot see every `rs.status()` detail with every user role. Use the Atlas UI metrics when the shell is limited.

Do not expose the cluster to `0.0.0.0/0` for convenience. Use an IP access list or a private network.

Self-managed Community replica sets are still useful. They teach `rs.initiate()`, elections that you trigger, and hidden members. Atlas teaches the production shape.

### Questions

#### Theoretical questions

1. Who starts `mongod` on Atlas?
2. Why does an Atlas URI use SRV?
3. Does Atlas remove the need to set write concern?
4. How is Atlas backup different from a second Atlas secondary?
5. Why is the free tier still a replica set for learning?

#### Easy practical tasks

1. Open your Atlas cluster. Write the member count that the UI shows.
2. Copy the SRV URI shape without the password. Write the hostname pattern.
3. Open Atlas replica-set documentation. Write the URL.
4. Make a table: task, Atlas UI or `mongosh`. Add failover, oplog size, add user, insert document.

#### Medium practical tasks

1. Insert with majority write concern on Atlas. Find the document. Write the concern that you used.
2. Trigger a test failover in Atlas if your tier allows it, or read the failover doc and write the expected client effect.
3. Compare a local three-node Docker set with Atlas: who changes disk, who changes TLS, who changes members.

#### Advanced practical tasks

1. Read Atlas multi-region replica-set options. Write how priority and read preference tags would apply.
2. Write a one-page runbook: Atlas failover, how the app retries, who you page, which metric is lag.
...
4. How do stale secondary reads and Atlas automatic failover create different application risks?
