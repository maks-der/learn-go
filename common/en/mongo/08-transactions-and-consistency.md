...
This topic shows how MongoDB keeps writes correct. A single document write is always atomic. Multi-document transactions exist on replica sets and on sharded clusters. You learn when you do not need a transaction. You set read concern, write concern, and read preference. You learn causal consistency at a high level.

Complete CRUD, modeling, and drivers first. Complete this topic before you study replica sets in detail.
...
4. Why can a secondary miss a write that just succeeded?
...
3. Run `hello` or `rs.status()`. Write which member is primary. Then set read preference `secondary` and run a find.
4. Make a table: mode, can be stale, typical use. Add the five read preference modes.

#### Medium practical tasks

1. Compare latency of `w: 1` vs `w: "majority"` on 100 inserts. Write the two times.
2. Insert on primary. Immediately read with `secondary`. Repeat until you see lag or write that you did not see lag. Write the observation.
...
2. Read about max staleness (`maxStalenessSeconds`). Write how it limits how stale a secondary can be.
