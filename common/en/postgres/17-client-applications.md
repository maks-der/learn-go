...
Complete topic 16 first. You know that a replica is read-only. Complete this topic before you change memory settings.
...
## Connection strings and `libpq` parameters
...
## Pooling: PgBouncer (transaction vs session)
...
PgBouncer authenticates clients and opens server connections as configured. You still use a least-privilege role (topic 12). You still limit `max_connections` on PostgreSQL (topic 18 and topic 19).
...
Official site: [https://www.pgbouncer.org/](https://www.pgbouncer.org/).
...
**`pgx`** is a common Go driver. It can use the protocol without libpq. Connection strings still look like libpq URIs. Topic paths in this repo that use Go should prefer `pgx` with a pool (`pgxpool`) in the application, plus PgBouncer when many app hosts share one cluster.
...
- request `sslmode` that matches topic 12
...
## Prepared statements and `PREPARE`
...
Do not concatenate user text into `PREPARE` (that misses the point). Do not assume a prepared statement is a cache across PgBouncer transaction mode. Do not keep thousands of named prepares in a long session without `DEALLOCATE`.
...
In `psql`, session A prints an asynchronous notice after session B commits (a simple `NOTIFY` auto-commits). In a driver, you must poll or use the driver API for notifications.

Rules:

- channel names are identifiers
- the payload is `text` (length limit applies; keep it small; put a key, not a document)
...
Do not use `NOTIFY` as the only store of an order. Do not send megabytes in the payload. Do not `LISTEN` on a new connection per HTTP request.

Official: [https://www.postgresql.org/docs/17/sql-notify.html](https://www.postgresql.org/docs/17/sql-notify.html), [https://www.postgresql.org/docs/17/sql-listen.html](https://www.postgresql.org/docs/17/sql-listen.html).
...
3. Why keep the payload small?
4. Which pool mode breaks `LISTEN`?
5. Why do you still need a table for the work?

#### Easy practical tasks

1. Open two `psql` sessions. `LISTEN` in A. `NOTIFY` in B. Write what A prints.
2. `BEGIN; NOTIFY ...; ROLLBACK;` in B. Write whether A received the payload.
3. Query `pg_listening_channels();` in the listener session.
4. Write four sentences: channel, payload, commit, table.

#### Medium practical tasks

1. `NOTIFY` with a payload that is a row id. In A, `SELECT` that row from a table after the notice.
2. Try `LISTEN` through a driver example from the driver docs. Write the API names.
3. Document why a serverless function that dies after 100 ms is a poor listener.

#### Advanced practical tasks

1. Write a small worker loop: `LISTEN`, wait, `SELECT` unprocessed rows, update them. Use two sessions.
2. Read notify payload limits in the 17 docs. Write the limit and a team rule for payload content.

---

## COPY for bulk load

`COPY` moves rows between a table and a file or a stream. It is the fast path for bulk load and unload. It is one transaction unless you split files.
...
Binary format is faster for large numeric tables. CSV is easier to debug. `NULL` representation must match the file.

`COPY` does not fire per-row `BEFORE INSERT` triggers in a cheaper way that skips them; row triggers still run. Statement triggers and constraints still apply. Disable indexes or constraints only with a written plan.

Do not build 100000 single-row `INSERT` statements if `COPY` is available. Do not run server-side `COPY` from a path that the `postgres` OS user must not read. Do not load CSV without a header rule and a column list.

Official: [https://www.postgresql.org/docs/17/sql-copy.html](https://www.postgresql.org/docs/17/sql-copy.html).

### Questions

#### Theoretical questions

1. What is the difference between `COPY` and `\copy`?
2. Who reads the file path in SQL `COPY FROM '/path'`?
3. Why pass a column list?
4. Does `COPY` skip row triggers?
5. When is binary format useful?

#### Easy practical tasks

1. `\copy` two rows from a small CSV into a table. Select them.
2. `\copy` the table out to a new CSV. Open the file.
3. Run `\h COPY` and write two `WITH` options.
4. Write four sentences: stream, client file, server file, column list.

#### Medium practical tasks

1. Compare wall time of 1000 single `INSERT`s versus one `COPY` of 1000 rows (lab). Write the two times.
2. Load CSV with a wrong `NULL` token. Record the error. Fix the `WITH` clause.
3. Use a driver COPY API (psycopg `copy`, pgx `CopyFrom`) if you can. Write the function name.

#### Advanced practical tasks

1. `COPY` into a table with a `BEFORE INSERT` trigger. Prove the trigger ran (for example `updated_at` or an audit row).
2. Read `COPY` permissions in the 17 docs. Write who may `COPY` to a server file and what safer alternative you use in apps.
...
1. Connect with a URI that sets `application_name`. Run `SELECT 1`. Run a `\copy` of one table.
2. Write a cheat sheet: libpq keys, three pool modes, three drivers, `PREPARE`, `LISTEN`, `COPY` versus `\copy`.
3. `PREPARE` and `EXECUTE` one `SELECT`. `DEALLOCATE`.
4. Open two sessions and complete one `LISTEN` / `NOTIFY` round trip.

#### Medium practical tasks

1. Write an app connection policy: URI fields, pool mode, driver, max pool size versus `max_connections`.
2. Load 5000 CSV rows with `\copy`. Then `LISTEN` is not required. Write timings and `pg_stat_activity` during the load.
3. Diagram two hosts of a Go API, PgBouncer transaction mode, and PostgreSQL. Count connections at each hop for 50 concurrent requests.

#### Advanced practical tasks

1. Build a small program that uses parameters, `COPY` in, and `RETURNING` on a single-row insert. Refuse a URI with a password.
2. Map this topic to official libpq, `PREPARE`, `NOTIFY`, and `COPY` pages. Add one libpq parameter that this topic did not list.
