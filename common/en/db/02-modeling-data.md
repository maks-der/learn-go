...
Data modeling is the work of naming the things you store and the links between them. This topic shows entities, attributes, keys, relationship types, and `NULL`.

Use one term for each concept. An entity is a thing you store. An attribute is a fact about that thing. A relationship is a link between entities. A primary key identifies one row. A foreign key points to another row. Complete this topic before you write SQL queries.
...
Name entities with nouns. Name relationships with verbs. Keep names singular in the model (`customer`, not `customers`) if your team uses that rule. Then choose table names with a team convention.
...
Each column has a data type. The type limits the values. Example: `email` is text. `id` is an integer. A later topic covers types in more detail.
...
The DBMS rejects an insert into `orders` when `customer_id` is not in `customers`. The DBMS also rejects a delete of a customer when orders still point to that customer, unless you define a cascade rule. The default safe action is to reject the delete.
...
2. Why must a primary key be unique and not `NULL`?
3. What does a foreign key point to?
4. What is referential integrity?
5. When may a foreign key be `NULL`?

#### Easy practical tasks

1. Write a `CREATE TABLE` for `products` with a primary key `product_id`.
2. Write a `CREATE TABLE` for `loans` with foreign keys to `members` and `books`.
3. Draw `customers` and `orders`. Mark the primary key and the foreign key.
4. Write one insert into `orders` that must fail because the customer does not exist.

#### Medium practical tasks

1. Design `order_lines` with a composite primary key. Write the columns and the key.
2. Create two tables with a foreign key. Insert a parent row. Insert a child row. Try a child row with a bad id. Record the error.
3. Try to delete the parent while a child exists. Record the result. Write the safe meaning of that result.

#### Advanced practical tasks

1. Model three tables in a chain (`countries`, `cities`, `addresses`). Write the primary keys and the foreign keys. State the delete problem if you remove a country.
...
SQL treats `NULL` in a special way. A comparison with `NULL` does not yield true. `price = NULL` is unknown. You test missing values with `IS NULL` and `IS NOT NULL`. A later topic covers those predicates.
...
Aggregate functions ignore `NULL` in general. `SUM` of `{10, NULL, 5}` is `15`. `COUNT(column)` does not count `NULL` in that column. A later topic covers this behavior.
